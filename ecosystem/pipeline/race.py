...
BINARY = ROOT / '_artifact' / 'pipeline-race-tests'


def main():
    if not GENERATED.exists():
        raise RuntimeError('Run goml test in ecosystem/pipeline before race.py')
    ...
    for name in tests:
        subprocess.run([str(BINARY), f'ecosystem::pipeline::tests::{name}'], cwd=ROOT, env=environment, check=True, timeout=30)
    print(f'Pipeline race detector: {len(tests)} tests passed')
