...
The executable runs a checked parse/read/rewrite example by default. `--json` accepts the deterministic tree and configuration request format used by the library's `interop.py`; it returns structural/range reports and edit results. Use `python3 ecosystem/verify.py syntax` at the repository root to provision its registry snapshot, format-check, build, test and run the consumer.
