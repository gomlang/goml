...
func Start(secure, mutual bool) Server {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/echo":
			...
			json.NewEncoder(w).Encode(map[string]string{"method": r.Method, "path": r.URL.Path, "query": r.URL.RawQuery, "body": string(body), "authorization": r.Header.Get("Authorization"), "cookie": r.Header.Get("Cookie"), "custom": strings.Join(r.Header.Values("X-Custom"), "|"), "content_type": r.Header.Get("Content-Type"), "user_agent": r.Header.Get("User-Agent")})
		case "/multipart":
	...
	result := &state{http: server, entered: entered}
	if secure {
