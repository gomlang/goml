...
use the standard I/O, resource, byte snapshot and derive capabilities introduced
in GoML 0.1.50. The web library additionally requires this checkout's development
toolchain for `std::panic` and panic-safe lexical cleanup; build it with `just make`
and use `stage2/bin` until those capabilities are released.
...
| [request](request/README.md) | GoML HTTP(S)/HTTP2 clients with pooled multiplexed connections, request builders, JSON/form/multipart, redirects, per-stream cancellation, TLS policy and bounded responses | 49 library tests and race, consumer and HTTP/TLS/proxy interoperability checks pass; native servers are test-only reference peers |
| [llvm](llvm/README.md) | LLVM 18 typed handles, SSA construction/editing, mutable-local promotion, IR traversal, target machines, ABI layouts, optimization and cross-target output | Implemented; 19 library tests, 12 native tests, 5 consumer tests, race checks, four target formats and 12,420 linked-function comparisons pass |
...
| [web](web/README.md) | GoML HTTP/1.1 server routing, typed request extraction, middleware, streaming I/O/SSE, panic isolation, cancellation, bounds and graceful shutdown | 32 library tests, 5 consumer tests, live HTTP interoperability and race checks pass |
...
| [color](color/README.md) | Checked sRGB/linear/HSL/HSV/XYZ/Lab/Oklab conversions, CSS colors, alpha compositing, gamut mapping, contrast, Delta E and gradients | Pure GoML over standard math; 22 library tests, consumer tests and 4,659 numerical reference cases pass |
...
| [archive](archive/README.md) | GoML USTAR/PAX and classic ZIP codecs, CRC32, bounded TAR streaming and ZIP indexing, DEFLATE/GZIP codecs, metadata and rooted extraction with an openat fallback for older kernels | 17 library tests, GNU gzip/tar/Info-ZIP interoperability, consumer and race checks pass |
