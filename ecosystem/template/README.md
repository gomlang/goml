...
```sh
python3 ecosystem/verify.py template
python3 ecosystem/template/interop.py
```

There are 11 external library tests and 2 independently resolved consumer tests.
...
The differential harness compares 1,367 shared-syntax cases with checksum-pinned
...
escaping/captures, filtered loops, includes and multilevel inheritance. Reference
packages are extracted only under `ecosystem/_artifact/reference/`; they are not
runtime dependencies or installed globally. Reports and failures are retained
under `ecosystem/_artifact/verification/template/`.
