...
`from_reader[R: std::io::Read](reader, max_bytes)` reads incrementally, preserves incomplete UTF-8 scalars across read boundaries, and reports absolute malformed-byte offsets. It validates returned byte counts, rejects truncated UTF-8 at EOF, and enforces the caller's byte limit. At the limit it may consume one extra byte to distinguish EOF from oversized input. Every read error, including `Interrupted`, returns immediately without automatic retry and retains the original `std::io::Error` in `Error::Io`. No partial rope is returned. The failing call may have consumed input or modified scratch storage without reporting a count; those bytes are not appended. Calling `from_reader` again on the same source does not restore earlier input or partial UTF-8 state, so recovery requires caller-owned source positioning or a fresh source.

`write_to[W: std::io::Write]` writes chunks through `write_all`, completing partial writes and returning errors including interruptions without automatic replay. It does not flush or close the caller's stream. Errors may occur after partial input consumption or output delivery, and the failing write call may have unreported side effects.
