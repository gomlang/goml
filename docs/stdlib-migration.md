...
All 74 capability rows are accepted against their mapped GoML contracts. No
rows remain in progress or planned-only. The current batch below adds 32
locally accepted rows.
...
release objective; publication remains a separate gate.
...
### Accepted current implementation batch

The following 32 rows have source, public consumers and focused tests. The
ecosystem modules passed the sibling verification repository's isolated `just ecosystem-test` checks;
standard packages passed fixtures, source catalog and navigation checks,
stage2 construction and full `just ci`. The CI run passed 1,033 compiler tests,
174 driver tests, bootstrap fixed-point verification and packaging checks with
Go 1.26.0. No row in this table implies publication or migration of
self-hosted consumers.

| Rows | Implemented GoML contract and focused evidence |
| --- | --- |
| `text/scanner`; `text/tabwriter` | `ecosystem::parser::scanner` provides bounded Unicode-aware tokens, positions and sticky errors; `ecosystem::tabwriter` streams aligned tab blocks with byte or terminal-column width. Parser and tabwriter module/consumer verification passed. |
| `regexp/syntax`; `regexp` | `ecosystem::regexp` parses a bounded AST, compiles a Thompson NFA, and exposes UTF-8 search, captures, replacement and split with shared work/output limits. It documents leftmost-longest matching and the supported grammar; regexp module/consumer verification passed. |
| `html/template` | The existing `ecosystem::template` engine gained contextual HTML, attribute, URL, JS and CSS rendering with conservative rejection of unsupported active contexts. Template module/consumer verification passed. |
| `log/slog`; `testing/slogtest` | `ecosystem::tracing` gained checked structured logger fields/groups and sink wrappers; `ecosystem::tracing::testing` probes handler ordering, forwarding, flush and finish. Tracing module/consumer/race verification passed. |
| `testing/iotest`; `math/cmplx`; `math/rand/v2` | `std::testing::io` supplies fragmented/short I/O and delayed-error fixtures; `std::math::complex` provides f64 value arithmetic and principal functions; `std::rand` adds PCG, ChaCha8 and unbiased stateful sampling without changing splitmix64-v1. Module fixtures 093, 099 and 100 passed. |
| `crypto/sha256`; `crypto/sha512` | `std::crypto::hash` now implements all six SHA-2 variants incrementally in pure GoML while preserving the existing `sha256` API. Project101 passed known-answer, padding-boundary, chunk/copy/reset and Go 1.26 differential tests; its golden output was generated by `just update-golden`. |
| `crypto/hmac`; `crypto/hkdf` | New `std::crypto::hmac` and `std::crypto::hkdf` packages use caller-supplied fresh digest factories, non-consuming sums, checked RFC 5869 output bounds and reusable HMAC state. Project102 passed RFC vectors, Go 1.26 differential tests and boundary checks; its golden output was generated by `just update-golden`. |
| `crypto/sha3`; `crypto/pbkdf2` | `std::crypto::hash` adds pure SHA-3/SHAKE with explicit absorb/squeeze and per-call output bounds; `std::crypto::pbkdf2` reuses HMAC with checked output, iteration and PRF-work limits. Project103 passed NIST/RFC vectors, Go 1.26 differential tests, lifecycle and limit checks; its golden output was generated by `just update-golden`. |
| `net/mail` | `ecosystem::mail` parses bounded modern address lists and folded/duplicate headers on MIME and textproto, with explicit body offsets and error positions. Mail module/consumer verification passed; SMTP and obsolete route syntax remain outside the mapped contract. |
| `encoding/asn1` | `ecosystem::asn1` provides bounded canonical DER/OID encoding and decoding with explicit `Value` and `Schema` trees. ASN.1 module/consumer verification passed; supported tags and resource limits are documented in its README. |
| `crypto/x509/pkix` | `ecosystem::x509::pkix` uses extended bounded ASN.1 TLV validation for RDN Names, OIDs, Extensions and AlgorithmIdentifiers while preserving unknown explicit DER values. X509 and ASN.1 module/consumer verification passed; certificate chain and trust policy remain outside the mapped contract. |
| `time/tzdata` | `ecosystem::datetime::tzdata` embeds 598 verified TZif zones/aliases from the pinned 2026c snapshot and loads them through the existing timezone parser. Datetime module/consumer/race verification passed, including gap/fold, skipped midnight and historical-offset cases. |
| `image/color`; `image/color/palette` | `ecosystem::color` gained explicit straight/premultiplied pixel conversions and immutable palettes with linear-premultiplied nearest-color lookup. Color module/consumer verification passed. |
| `image`; `image/draw` | `ecosystem::image` adds bounded premultiplied RGBA pixel buffers, checked half-open rectangles, copied subimages and a codec contract; `ecosystem::image::draw` clips copy/source-over operations and snapshots overlapping sources. Image module/consumer verification passed. |
| `image/png` | `ecosystem::image::png` decodes every standard color type and legal bit depth, filters 0–4, palette/transparency and Adam7 through bounded zlib, and writes RGBA8 PNG. Image module/consumer verification passed, including malformed inputs and Go 1.26 decoding of emitted PNG. |
| `math/big` | `ecosystem::bigmath` provides bounded exact Rational and explicit-context binary Float over `ecosystem::bigint`, with 1–4096-bit precision and checked exponent/resource errors. Bigmath module/consumer verification passed against 496 frozen Go `math/big` vectors; bigint regression and race checks passed. |
| `encoding/xml` | `ecosystem::xml` offers bounded incremental XML 1.0 tokens, namespace resolution, escaping and an explicit flat-struct serde schema. XML module/consumer verification passed; DTDs and external entities are rejected. |
| `go/doc/comment` | `ecosystem::go_doc` parses a bounded comment AST and renders Comment, Text, Markdown and escaped HTML. Module/consumer verification passed, including link-definition budget checks; source extraction and symbol resolution remain outside its documented API. |
| `debug/elf` | `ecosystem::object::elf` reads bounded ELF32/64 in both byte orders, sections/programs, static/dynamic symbols and canonical extended numbering. Object module/consumer verification passed with four Go 1.26 `debug/elf` reference matrices and malformed-input/resource tests. |
| `debug/dwarf` | `ecosystem::dwarf` reads bounded DWARF32 v2–v5 units, common forms and line tables through random-access I/O. Its cumulative decoded-text budget, unit references and file-count limits have regression coverage; module/consumer verification passed against Go 1.26 `debug/dwarf` fixtures. |
| `database/sql` | `ecosystem::sql` provides typed rows, queries, cursors and transactions with a SQLite adapter and bounded serial pool. The pool rejects dirty leases and invalidates retained connection aliases; SQL module/consumer and existing SQLite regression verification passed. |
| `crypto` | `std::crypto` now exposes full-length digest byte comparison and generic `Hasher` verification, reused by HMAC. Project102, source-world selection, navigation and full CI passed; the compiler does not enforce constant-time execution. |
...
This local acceptance does not imply publication.
...
checks for both modules. This acceptance records only the TAR capability; the
ZIP capability has its own acceptance record below.
