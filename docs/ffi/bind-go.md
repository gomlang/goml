...
Each entry may have `type_arguments`, a finite array using the existing bridge-type JSON schema. For example, `[{"tag":"int64"}]` requests one concrete Go `int64` argument. The Go checker validates instantiation and constraints. Function queries preserve both the original declaration signature and the concrete signature; GoML substitutes the requested arguments and verifies that the signatures agree.
